...
resource "aws_subnet" "subnet" {
  ...
  availability_zone = "${data.aws_region.current.region}${var.subnet_group_properties["availability_zones"][count.index]}"

  tags = merge(tomap({"Name" = "${var.subnet_group_properties["subnet_group_name"]}-${data.aws_region.current.region}${var.subnet_group_properties["availability_zones"][count.index]}.${var.vpc_name}"}),
